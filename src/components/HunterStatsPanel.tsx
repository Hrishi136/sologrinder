...
export default function HunterStatsPanel({ stats }: { stats: Stat[] }) {
  return (
    <div className="grid grid-cols-3 gap-2 sm:gap-4 items-end w-full overflow-hidden">
      {stats.map(stat => {
        ...
        return (
          <div key={stat.label} className="flex flex-col items-center min-w-0 max-w-full">
            ...
            <div className="relative w-6 sm:w-12 h-16 sm:h-24 flex items-end mb-1 sm:mb-2 overflow-hidden">
              ...
              <div 
                className="absolute bottom-0 left-1/2 -translate-x-1/2 w-2 rounded-full bg-gradient-to-t from-system-blue2 to-system-blue"
                style={{ 
            ...
            <span className="font-orbitron text-white text-xs sm:text-base">{displayVal}</span>
          </div>
        );
      })}
    </div>
