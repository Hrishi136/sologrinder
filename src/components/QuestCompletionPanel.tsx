...
const DIFFICULTIES = [
  { key: "easy", label: "Easy", color: "#48e18b" },
  { key: "medium", label: "Medium", color: "#f4e95a" },
  { key: "hard", label: "Hard", color: "#ed3434" },
];

export default function QuestCompletionPanel({
  streak, dailyQuests, canCompleteQuest, completeQuest, setSystemNotice, QUEST_CATEGORIES
}: Props) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mt-3">
        <button
          className={`glow-button text-base py-1 px-3 flex-1 ${canCompleteQuest(difficulty) ? '' : 'opacity-30 pointer-events-none'}`}
          style={{ background: DIFFICULTIES.find(d=>d.key===difficulty)?.color }}
          onClick={() => {
            if (completeQuest(cat, difficulty))
